...
IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND name = 'spGetPTHTDX')
   
exec('CREATE PROCEDURE spGetPTHTDX AS BEGIN SET NOCOUNT ON; END')
...
IF @IsLock = -1
INSERT INTO #DXMH(MS_DE_XUAT, TRANG_THAI)
...
WHERE (T1.NGAY_LAP BETWEEN @TuNgay AND @DenNgay)
			AND (T1.MS_TO = @PhongBan OR  @PhongBan = -1 )
			AND (T1.TRANG_THAI = @IsLock OR @IsLock = -1) 
			AND (T2.GROUP_ID = @GroupID)
...
BEGIN
	...
	(
			...
			UNION
			SELECT DISTINCT T1.MS_DE_XUAT,'DX', 1 AS SL_NT FROM #DXMH T1 INNER JOIN dbo.DE_XUAT_MUA_HANG_DICH_VU T2 ON T2.MS_DE_XUAT = T1.MS_DE_XUAT
	...
	(
			...
			UNION
			SELECT DISTINCT T1.MS_DE_XUAT,'DX', 1 AS SL_NT FROM #DXMH T1 INNER JOIN dbo.DE_XUAT_MUA_HANG_DICH_VU T2 ON T2.MS_DE_XUAT = T1.MS_DE_XUAT WHERE T1.TRANG_THAI = 4
