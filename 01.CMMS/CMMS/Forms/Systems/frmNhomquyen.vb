...
#Region "Xu ly Luu/Xoa"
    ...
    Private Sub grdDanhsachuser_InitNewRow(sender As Object, e As InitNewRowEventArgs) Handles grdDanhsachuser.InitNewRow
        ...
        grdDanhsachuser.SetFocusedRowCellValue("ACTIVE", 1)
...
#Region "Xu ly Tab Menu"
    ...
    Private Sub XtraTabControl1_SelectedPageChanged(sender As Object, e As DevExpress.XtraTab.TabPageChangedEventArgs) Handles XtraTabControl1.SelectedPageChanged
        Try
            ...
            If XtraTabControl1.SelectedTabPage.Name = "TabForm" Then
                ...
                End If
                If XtraTabControl3.SelectedTabPage.Name = "Tab_Menu" Then
                    LoadGroupMenu(txtGroupID.Text)
                End If
            End If
    ...
    Private Sub grdQuyentrenmenu_CellValueChanged(sender As Object, e As DevExpress.XtraGrid.Views.Base.CellValueChangedEventArgs)
        Try
        ...
        Try
            If isLoadReport = True Then
                ...
                Else
                    ...
                    End If
                End If
                grdListofReport.Columns("REPORT_MAIL").GroupIndex = 0
                grdListofReport.Columns("TEN_L_REPORT").GroupIndex = 1
                'grdListofReport.Columns("TEN_L_REPORT").GroupFormat
...
#Region "Xu ly Tab nguoi dung"
    ...
    Private Sub LoadListAllUser()
        ...
        Try
            ...
            sSql = " SELECT A.USERNAME, A.FULL_NAME, B.GROUP_NAME, B.DESCRIPTION , A.GROUP_ID, A.GROUP_ID,HO + ' ' + TEN AS HO_TEN , D.TIME_LOGIN" &
                    " FROM dbo.USERS AS A INNER JOIN dbo.NHOM AS B ON A.GROUP_ID = B.GROUP_ID  " &
                    " LEFT JOIN CONG_NHAN C ON A.MS_CONG_NHAN = C.MS_CONG_NHAN LEFT JOIN (SELECT USER_LOGIN,TIME_LOGIN FROM  dbo.LOGIN UNION  SELECT Username,TimeLogin FROM dbo.WebUserLogin) D ON A.USERNAME = D.USER_LOGIN " &
    ...
    Private Sub ShutDownUserToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles ShutDownUserToolStripMenuItem.Click
        With grdListUser
            ...
            If Traloi = vbYes Then
                sSql = "DELETE  dbo.LOGIN WHERE USER_LOGIN = '" & .GetRowCellValue(.FocusedRowHandle, "USERNAME").ToString() & "' DELETE  dbo.WebUserLogin WHERE Username = '" & .GetRowCellValue(.FocusedRowHandle, "USERNAME").ToString() & "'"
...
#End Region
    ...
    Private Sub SetFormPermission(ByVal FORM_NAME As String, ByVal USER_NAME As String)
        ...
        End Using
    End Sub
    'tìm kiếm theo nhóm người dùng
    Private Sub txtSNhom_EditValueChanged(sender As Object, e As EventArgs) Handles txtSNhom.EditValueChanged
        Dim dtTmp As DataTable = New DataTable()
        Dim sdkien As String = "( 1 = 1 )"
        Try
            dtTmp = CType(GridControl1.DataSource, DataTable)
            If txtSNhom.Text.Length <> 0 Then sdkien = " ( " & " DESCRIPTION LIKE '%" & txtSNhom.Text & "%' OR GROUP_NAME LIKE '%" + txtSNhom.Text & "%') "
    ...
    Private Sub TextEdit2_EditValueChanged(sender As Object, e As EventArgs) Handles TextEdit2.EditValueChanged
        ...
        Dim sdkien As String = "( 1 = 1 )"
        Try
            dtTmp = CType(GridControl2.DataSource, DataTable)
            If TextEdit2.Text.Length <> 0 Then sdkien = " ( " & " USERNAME LIKE '%" & TextEdit2.Text & "%' OR FULL_NAME LIKE '%" + TextEdit2.Text & "%' OR DESCRIPTION LIKE '%" + TextEdit2.Text & "%') "
            dtTmp.DefaultView.RowFilter = sdkien
        Catch
            Try
                dtTmp.DefaultView.RowFilter = ""
    ...
    Private Sub txt_TimMenu_EditValueChanged(sender As Object, e As EventArgs)
        Dim dtTmp As DataTable = New DataTable()
        Dim sdkien As String = "( 1 = 1 )"
        Try
            dtTmp = CType(GridControl3.DataSource, DataTable)
            If txt_TimMenu.Text.Length <> 0 Then sdkien = " ( " & " MENU_NAME LIKE '%" & txt_TimMenu.Text & "%') "
            dtTmp.DefaultView.RowFilter = sdkien
        Catch
            Try
                dtTmp.DefaultView.RowFilter = ""
    ...
    Private Sub txt_TimForm_EditValueChanged(sender As Object, e As EventArgs) Handles txt_TimForm.EditValueChanged
        Dim dtTmp As DataTable = New DataTable()
        Dim sdkien As String = "( 1 = 1 )"
        Try
            dtTmp = CType(GridControl4.DataSource, DataTable)
            If txt_TimForm.Text.Length <> 0 Then sdkien = " ( " & " TEN_FORM LIKE '%" & txt_TimForm.Text & "%') "
            dtTmp.DefaultView.RowFilter = sdkien
        Catch
            Try
                dtTmp.DefaultView.RowFilter = ""
    ...
    Private Sub timkiem(ByVal cot As String, ByVal grid As GridControl, ByVal txt_tk As TextEdit)
        Dim dtTmp As DataTable = New DataTable()
        Dim sdkien As String = "( 1 = 1 )"
        Try
            dtTmp = CType(grid.DataSource, DataTable)
            If txt_tk.Text.Length <> 0 Then sdkien = " ( " & cot & " LIKE '%" & txt_tk.Text & "%') "
            dtTmp.DefaultView.RowFilter = sdkien
        Catch
            Try
                dtTmp.DefaultView.RowFilter = ""
        ...
        End Try
    End Sub
    Private Sub txt_TimDiaDiem_EditValueChanged(sender As Object, e As EventArgs) Handles txt_TimDiaDiem.EditValueChanged
        timkiem("TEN_N_XUONG", GridControl5, txt_TimDiaDiem)
    End Sub

    Private Sub txt_TimDayChuyen_EditValueChanged(sender As Object, e As EventArgs) Handles txt_TimDayChuyen.EditValueChanged
        timkiem("TEN_HE_THONG", GridControl6, txt_TimDayChuyen)
    End Sub


    Private Sub txt_TimLoaiTB_EditValueChanged(sender As Object, e As EventArgs) Handles txt_TimLoaiTB.EditValueChanged
        timkiem("TEN_LOAI_MAY", GridControl7, txt_TimLoaiTB)
    End Sub

    Private Sub txt_TimBPCP_EditValueChanged(sender As Object, e As EventArgs) Handles txt_TimBPCP.EditValueChanged
        timkiem("TEN_BP_CHIU_PHI", GridControl8, txt_TimBPCP)
    End Sub

    Private Sub txt_TimLoaiCV_EditValueChanged(sender As Object, e As EventArgs) Handles txt_TimLoaiCV.EditValueChanged
        timkiem("TEN_LOAI_CV", GridControl9, txt_TimLoaiCV)
    End Sub
    Private Sub txt_TimVTPT_EditValueChanged(sender As Object, e As EventArgs) Handles txt_TimVTPT.EditValueChanged
        timkiem("TEN_LOAI_PT", GridControl10, txt_TimVTPT)
    End Sub

    Private Sub txt_TimKho_EditValueChanged(sender As Object, e As EventArgs) Handles txt_TimKho.EditValueChanged
        timkiem("TEN_KHO", GridControl11, txt_TimKho)
    End Sub

    Private Sub txt_TimPhongBan_EditValueChanged(sender As Object, e As EventArgs) Handles txt_TimPhongBan.EditValueChanged

        timkiem("TEN_DON_VI", GridControl12, txt_TimPhongBan)
    End Sub

    Private Sub txt_TimBaoCao_EditValueChanged(sender As Object, e As EventArgs) Handles txt_TimBaoCao.EditValueChanged
        timkiem("TEN_REPORT", GridControl15, txt_TimBaoCao)
    End Sub

    Private Sub grdListofReport_CustomDrawGroupRow(sender As Object, e As Views.Base.RowObjectCustomDrawEventArgs) Handles grdListofReport.CustomDrawGroupRow
        Dim view As GridView = TryCast(sender, GridView)
        ...
        Try
            ...
            info.GroupText = info.EditValue()
            'nếu là một 
            If info.Column.Name = "REPORT_MAIL" Then
                Select Case info.EditValue()
                    Case 1
                        info.GroupText = Commons.Modules.ObjLanguages.GetLanguage(Me.Name, "sBaoCao")
                    Case 2
                        info.GroupText = Commons.Modules.ObjLanguages.GetLanguage(Me.Name, "sBaoCaoGuiMail")
                    Case Else
                        info.GroupText = Commons.Modules.ObjLanguages.GetLanguage(Me.Name, "sCheDoHienThi")
                End Select
            Else
                info.GroupText = info.EditValue()
            End If
