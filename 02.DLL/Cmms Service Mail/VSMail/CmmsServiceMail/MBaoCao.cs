...
//@MS_LOAI_CV INT
                TbTSGSTT.Load(SqlHelper.ExecuteReader(Commons.IConnections.ConnectionString, "SP_Y_GET_TSGSTTDHKT", sUserName,
                    NNgu, TNgay, DNgay, "-1", -1, "-1", sThietBi, "-1", "-1", "-1",-1));
        ...
        public Boolean InVatTuXuatKho(DataTable dtSch, string paths)
        {
            DateTime TNgay, DNgay;
            string dangxuat = "";
            string tendx = "";
            ...
            if (dtSch.Rows[0]["DK_BAOCAO"].ToString() != "")
            {
                ...
                string[] chuoi_tach = sStmp.Split(new Char[] { '!' });
                msKho = chuoi_tach[0];
                dangxuat = chuoi_tach[1];
                tendx = chuoi_tach[2];
                SauNgayBD = chuoi_tach[3] == "1" ? true : false;
                iSNgay = Convert.ToInt32(chuoi_tach[4]);
            }
            if (SauNgayBD)
            {
                TNgay = DateTime.Now.Date;
            ...
            Commons.frmrptVAT_TU_XUAT_KHO bc = new Commons.frmrptVAT_TU_XUAT_KHO();
            return bc.GoiMailTuDong(paths, msKho, dangxuat, tendx, TNgay, DNgay);

        }

        public bool InNhapXuatTon(DataTable dtSch, string paths)
        {
            DateTime TNgay, DNgay;
            string slvt = "";
            ...
            if (dtSch.Rows[0]["DK_BAOCAO"].ToString() != "")
            {
                ...
                msKho = chuoi_tach[0];
                slvt = chuoi_tach[1];
                SauNgayBD = chuoi_tach[2] == "1" ? true : false;
                iSNgay = Convert.ToInt32(chuoi_tach[3]);
            }
            if (SauNgayBD)
            {
                TNgay = DateTime.Now.Date;
            ...
            ReportMain.UAC.ucNhapXuatTon uc = new ReportMain.UAC.ucNhapXuatTon();
            return uc.GoiMail(paths, msKho, TNgay, DNgay, Commons.Modules.TypeLanguage, slvt);
        }
        ...
        #region     "frmPhanTichNNNMTheoNN -- PTTGNMTMThang"
        ...
        #region "IN PTTGNMayVeCo"
        public Boolean PhanTichTGNMTMThangVECO(DataTable dtSch,
DevExpress.XtraGrid.GridControl grd, DevExpress.XtraGrid.Views.Grid.GridView grv, out int iCoDuLieu, out string sLoi, string sFile, GridControl grdData, GridControl grdData1, GridView grvData, GridView grvData1)
        {
            iCoDuLieu = 1;
            sLoi = "";
            string loi = "";
            ...
            try
            {
                ...
                string sTenHeThong = "", tenLoaiMay = "";

                sSql = "SELECT CASE " + NNgu.ToString() + " WHEN 0 THEN TEN_N_XUONG WHEN 1 THEN TEN_N_XUONG_A ELSE TEN_N_XUONG_H END AS TEN_N_XUONG, MS_N_XUONG FROM NHA_XUONG ";
                ...
                else
                {
                    ...
                    if (dtMay.Rows.Count != 0)
                    {
                        sTenHeThong = dtMay.Rows[0]["TEN_HE_THONG"].ToString();
                    }

                }
                if (sLMay == "-1") tenLoaiMay = " < ALL > ";
                else
                {
                    sSql = "SELECT TEN_LOAI_MAY FROM LOAI_MAY WHERE MS_LOAI_MAY = '" + sLMay + "' ";
                    dtMay = new DataTable();
                    ...
                    if (dtMay.Rows.Count != 0)
                    {
                        tenLoaiMay = dtMay.Rows[0]["TEN_LOAI_MAY"].ToString();
                    }
                        ...
                        try
                        {
                            ...
                            if (dr["INVECO"].ToString() == "7")
                            {
                                if (WinsorGroup != "-1")
                                {
                                    ...
                                    dtData.Load(SqlHelper.ExecuteReader(Commons.IConnections.ConnectionString, "GetNNNMTheoNNVECO", sUserName, WinsorGroup, iHThong, sLMay, "-1", TNgay, DNgay, NNgu, 0));
                                    ...
                                    if (dtData.Rows.Count <= 0)
                                    {
                                        ...
                                        if (num == 0)
                                        {
                                            Commons.Modules.ObjSystems.MLoadXtraGrid(grdData, grvData, dtData, false, true, false, true, true, "frmPhanTichNNNMTheoNNVeCo");
                                        ...
                                        xlWSheetRong.Name = "Empty Data - " + TNgay.AddDays(10).ToString("MMMM") + "_WinsorGroup";
                                    ...
                                    loi += " ---- " + InNNNgungMay(grdData, grdData1, grvData, grvData1, grv, xlApp1, excelWorkbooks, ref xlWorkBook, dtData, TNgay, DNgay, "-1", iLoaiBC, WinsorGroup, "WindsorGroup", iHThong, sTenHeThong, sLMay, tenLoaiMay, sPath, iLoaiBC, target, num);
                                    num++;
                            ...
                            {
                                dtData = new DataTable();
                                dtData.Load(SqlHelper.ExecuteReader(Commons.IConnections.ConnectionString, "GetNNNMTheoNNVECO", sUserName, dr["MS_N_XUONG"].ToString(), iHThong, sLMay, "-1", TNgay, DNgay, NNgu, 0));
                                Commons.Modules.ObjSystems.MLoadXtraGrid(grd, grv, dtData, false, true, true, true, true, "frmPhanTichNNNMTheoNN");
                                if (iLoaiBC == 0)
                                    dtData.DefaultView.Sort = "TG_NGUNG DESC";
                                else
                                ...
                                if (dtData.Rows.Count <= 0)
                                {
                                    ...
                                    xlWSheetRong = (Excel.Worksheet)xlWorkBook.Sheets[xlWorkBook.Sheets.Count];
                                    xlWSheetRong.Name = "Empty Data - " + TNgay.AddDays(10).ToString("MMMM") + "_" + (dr["MS_N_XUONG"].ToString().Contains(',') ? "WinsorGroup" : dr["MS_N_XUONG"].ToString());
                                    xlWSheetRong.Activate();
                                    num++;
                                    continue;
                                }
                                ...
                                loi += " ---- " + InNNNgungMay(grdData, grdData1, grvData, grvData1, grv, xlApp1, excelWorkbooks, ref xlWorkBook, dtData, TNgay, DNgay, "-1", iLoaiBC, dr["MS_N_XUONG"].ToString(), tenNXuong, iHThong, sTenHeThong, sLMay, tenLoaiMay, sPath, iLoaiBC, target, num);
                                num++;
                            }
                        }
                        catch (Exception ex) { sLoi += " - For in từng xưởng: " + ex.Message; }
                    }
            ...
            {
                sLoi = sLoi + " - " + ex.Message + " --" + loi;
                return false;
            }
        }

        private string InNNNgungMay(GridControl grdData, GridControl grdData1, GridView grvData, GridView grvData1, GridView grv, Excel.Application xlApp, Excel.Workbooks excelWorkbooks, ref Excel.Workbook xlWorkBook, DataTable dtData, DateTime tNgay, DateTime dNgay, string nnNMay, int loaiNN, object diaDiemID, object tenDiaDiem, object dayChuyenID, object tenDayChuyen, object loaiMayID, object tenLoaiMay, string sPath, int loaiBaoCao, double target, int num)
        {
            ...
            (title.FormatConditions.Add(Excel.XlFormatConditionType.xlCellValue, Excel.XlFormatConditionOperator.xlGreaterEqual, "=60", Type.Missing));
                condition1.Font.Color = System.Drawing.ColorTranslator.ToOle(System.Drawing.Color.Red);
                ...
                PhanTichTGNMTMThangLoadBieuDo(grvData, xlSheet, iDongBD, iTongDong, Convert.ToDouble(title.Left) + 15, Convert.ToDouble(title.Top), 386.92, 235.55, loaiBaoCao);
      ...
      "frmPhanTichNNNMTheoNNVeCo", "Sum", Commons.Modules.TypeLanguage), 2, 33 + grvData1.Columns.Count, "@", 10, true, Excel.XlHAlign.xlHAlignCenter, Excel.XlVAlign.xlVAlignCenter, true, 2, 33 + grvData1.Columns.Count);
                ...
                title.Formula = "=SUM(W3:X" + (grvData.RowCount + 2).ToString() + ")";

                title = Commons.Modules.MExcel.GetRange(xlSheet3, 1, 18, 1, 18);
                title.Formula = "=SUM(AH3:" + Commons.Modules.MExcel.TimDiemExcel(grvData.RowCount + 2, 32 + grvData1.Columns.Count) + ")";
        ...
        #region InDanhSachPhieuNhapKho
        public Boolean InDanhSachPhieuNhapKho(DataTable dtSch, out int iCoDuLieu, out string sLoi)
        {
            iCoDuLieu = 1;
            DataTable tbData = new DataTable();
            string sPath, SavePath;
            ...
            try
            {
                ...
                VietShape.ucDanhSachPhieuNhapKho mail = new VietShape.ucDanhSachPhieuNhapKho();
                mail.TNgay = Convert.ToDateTime(TNgay);
                mail.DNgay = Convert.ToDateTime(DNgay);
                mail.sMsLVT = "-1";
                mail.Uname = sUserName;
                mail.iMsKho = -1;
                mail.iNN = 1;
                mail.sTenKho = "ALL";
                mail.sTenLoaiVatTu = "ALL";

                SavePath = Application.StartupPath;
                if (mail.GoiMail(sPath, out sLoi))
                    return true;
                else
                {
                    iCoDuLieu = 0;
