using System;
using System.Collections.Generic;
using System.Text;
using Microsoft.ApplicationBlocks.Data;
using System.Data;
using CMMSIntegrationLib.CMMS;

namespace CMMSIntegrationLib.Integration
{
    public class integrate01_DVT_to_CMMS
    {
        public integrate01_DVT_to_CMMS()
        {
        }

        private DataTable dtDVTTransfer = new DataTable();
        private void GetDataTransfer(out string sLoi)
        {
            sLoi = "";
            try
            {
                dtDVTTransfer = new DataTable();
                string _sqlExec = "SELECT STT, DVT,DVT_CU, TEN_1, ISNULL(CONVERT(NVARCHAR(30),INSERT_DATE,121),'') AS INSERT_DATE , " +
                        " ISNULL(CONVERT(NVARCHAR(30),UPDATE_DATE,121),'') AS UPDATE_DATE,ISNULL(STATUS,'N') AS STATUS, " +
                        " ISNULL(DA_CHUYEN , CONVERT(BIT,0)) AS DA_CHUYEN, ISNULL(CONVERT(NVARCHAR(30),NGAY_CHUYEN,121),'') AS NGAY_CHUYEN " +
                        " FROM dbo.TB_DON_VI_TINH  WHERE ISNULL(DA_CHUYEN,CONVERT(BIT,0)) = CONVERT(BIT, 0)" +
                        " ORDER BY ISNULL(INSERT_DATE, GETDATE()), ISNULL(UPDATE_DATE, GETDATE()) ";
                dtDVTTransfer.Load(SqlHelper.ExecuteReader(integrate_common.cmms_connection, CommandType.Text, _sqlExec));
            }
            catch (Exception e)
            {
                sLoi = e.ToString();
            }
        }

        public string ExecSynchronizeDON_VI_TINH_To_DBCMMS()
        {
            string sLoi = "";
            try
            {
                GetDataTransfer(out sLoi);
                if (dtDVTTransfer.Rows.Count > 0)
                {
                    foreach (DataRow _rowtransfer in dtDVTTransfer.Rows)
                    {
                        DON_VI_TINH _item = new DON_VI_TINH();
                        _item.STT = int.Parse(_rowtransfer["STT"].ToString());
                        _item.DVT = VSCommons.MConvertFont.MVni2Uni(_rowtransfer["DVT"].ToString());
                        _item.DVT_CU = VSCommons.MConvertFont.MVni2Uni(_rowtransfer["DVT_CU"].ToString());
                        _item.TEN_1 = VSCommons.MConvertFont.MVni2Uni(_rowtransfer["TEN_1"].ToString());
                        _item.INSERT_DATE = _rowtransfer["INSERT_DATE"].ToString();
                        _item.UPDATE_DATE = _rowtransfer["UPDATE_DATE"].ToString();
                        _item.STATUS = _rowtransfer["STATUS"].ToString();
                        _item.DA_CHUYEN = (bool)_rowtransfer["DA_CHUYEN"];
                        _item.NGAY_CHUYEN = _rowtransfer["NGAY_CHUYEN"].ToString();
                        if (UpdateToDBCMMS(_item, out sLoi))
                            UpdateToDBIntegration(_item.STT.ToString());
                        else
                            sLoi += "\n" + sLoi;
                    }
                }
                else
                {
                    return "";
                }
            }
            catch (Exception e)
            {
                return e.ToString();
            }
            if (sLoi.Length > 0) return sLoi; else return "";
        }
        private bool UpdateToDBCMMS(DON_VI_TINH _item,out string sLoi)
        {
            sLoi = "";
            try
            {
                SqlHelper.ExecuteNonQuery(cmms_common.cmms_connection, "INTEGRATION_DON_VI_TINH_UPDATE",
                    _item.STT, _item.DVT, _item.DVT_CU, _item.TEN_1, _item.STATUS);
            }
            catch (Exception ex)
            {
                sLoi = _item.DVT + "-" + ex.Message;
                return false;
            }

            return true;
        }
        private bool UpdateToDBIntegration(string STT)
        {
        
            try
            {
                SqlHelper.ExecuteNonQuery(integrate_common.cmms_connection, CommandType.Text, "UPDATE TB_DON_VI_TINH SET DA_CHUYEN  = CONVERT(BIT,1) , NGAY_CHUYEN = GETDATE() WHERE STT = '" + STT.ToString() + "'");
            }
            catch
            {
                return false;
            }
            return true;
        }
   

    }
    public class DON_VI_TINH
    {
        public DON_VI_TINH() { }
        public int STT { get; set; }
        public string DVT { get; set; }
        public string DVT_CU { get; set; }
        public string TEN_1 { get; set; }
        public string INSERT_DATE { get; set; }
        public string UPDATE_DATE { get; set; }
        public string STATUS { get; set; }
        public bool DA_CHUYEN { get; set; }
        public string NGAY_CHUYEN { get; set; }
    }
}
