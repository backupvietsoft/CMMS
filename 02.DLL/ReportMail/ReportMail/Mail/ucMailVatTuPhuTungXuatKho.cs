...
{
    public partial class ucMailVatTuPhuTungXuatKho : UserControl
    {
        private string TenMBaoCao = "ucMailVatTuPhuTungXuatKho";
        private string DK_BAOCAO;
        csSchedules clsSchedules = new csSchedules();

        public ucMailVatTuPhuTungXuatKho()
        {
            InitializeComponent();
        }

        private void ucMailVatTuPhuTungXuatKho_Load(object sender, EventArgs e)
        {
            LoadDangXuat();
            LoadKho();
        ...
        private void LoadDangXuat()
        {
            ...
            {
                DataTable dtTmp = new DataTable();
                dtTmp.Load(SqlHelper.ExecuteReader(Commons.IConnections.ConnectionString, "GetDANG_XUATswithType", Commons.Modules.TypeLanguage));
                Commons.Modules.ObjSystems.MLoadCheckedComboBoxEdit(cboDangXuat, dtTmp, "MS_DANG_XUAT", "DANG_XUAT");
        ...
        private void LockControl(Boolean Locked)
        {
            ...
            cboDangXuat.Properties.ReadOnly = Locked;    //Enabled = !Locked;
        ...
        private void btnGhi_Click(object sender, EventArgs e)
        {
            ...
            if (cboDangXuat.EditValue == null || cboDangXuat.EditValue.ToString() == "")
            {
                XtraMessageBox.Show(Commons.Modules.ObjLanguages.GetLanguage(Commons.Modules.ModuleName, this.Name, "ChonDangXuat", Commons.Modules.TypeLanguage));
                return;
            }
            ...
            try
            {
                DK_BAOCAO = (cboKho.EditValue.ToString() == "" ? "" : cboKho.EditValue.ToString()) +
                    (cboKho.EditValue.ToString() == "" ? "" : "!") + cboDangXuat.EditValue.ToString() + "!" + Commons.Modules.ObjSystems.SplitString(cboDangXuat.Text) + "!" + (chkSau.Checked ? "1!" : "0!") + (txtSNgay.Text == "" ? "0" : txtSNgay.Text);
            }
            catch { DK_BAOCAO = ""; }
