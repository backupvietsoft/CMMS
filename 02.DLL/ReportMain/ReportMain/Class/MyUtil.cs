...
namespace ReportMain
{
    public class MyUtil
    {
        ...
        public MyUtil()
        {
            con = new SqlConnection(Commons.IConnections.ConnectionString);
        }
        //Kiem tra MSNX
        ...
        //Cap nhap CVBao tri
        public bool InsertWork(string macv, string kyhieucv, short ms_loai_cv, string motacv, string path, string ms_loai_may,
                                long thoi_gian_du_kien, string thaotac, string tieuchuan, string ghichu, int ms_chuyen_mon,
                                int ms_bac_tho, bool antoan, float dinh_muc, string ngoai_te, int item, int count, int MsCv,
                                System.Data.SqlClient.SqlConnection oCon, out string sMaLoi )
        {
            sMaLoi = "";
            try
            {
                ...
                command.Parameters.Add("@MA_CV", SqlDbType.NVarChar).Value = "NULL";
                ...
                command.Parameters.Add("@MS_CV", SqlDbType.NVarChar).Value = MsCv;
        ...
        public string KiemGetTonTai(string sTableKT, string sCotKT, string sGTKtra)
        {
            ...
            SqlCommand cmd = new SqlCommand();
            cmd.Connection = con;
            cmd.CommandText = "SELECT TOP 1 * FROM " +  sTableKT + " A WHERE " +  sCotKT + " = @name";
            cmd.Parameters.Add("@name", SqlDbType.NVarChar).Value = sGTKtra;
            cmd.CommandType = CommandType.Text;
            SqlDataReader dr = cmd.ExecuteReader();
            if (dr.HasRows)
            {
