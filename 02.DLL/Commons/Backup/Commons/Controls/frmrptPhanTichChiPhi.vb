...
Public Class frmrptPhanTichChiPhi
    ...
    Private Sub frmrptPhanTichChiPhi_Disposed(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Disposed
        Try
    ...
    Private Sub frmrptPhanTichChiPhi_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Commons.Modules.ObjSystems.DinhDang()
        vDenNgay = DateTime.Now.Date
        vTuNgay = vDenNgay.AddMonths(-1).Date
        mtxtTuNgay.Text = vTuNgay.ToString("dd/MM/yyyy")
        mtxtDenNgay.Text = vDenNgay.ToString("dd/MM/yyyy")
        vData = loadData()
        CreateData(vData)
        ViewData()
        RefreshLanguages()
    ...
    Private Sub CreateData(ByVal vtb As DataTable)
        ...
        Try
            ...
            " CHI_PHI_DV FLOAT, CHI_PHI_KHAC FLOAT,	CHI_PHI_HANG_NGAY FLOAT	, TONG FLOAT )"
        ...
        If (vtb.Rows.Count > 0) Then
            For Each vrow As DataRow In vtb.Rows
                Dim sqlInsert As String = " INSERT INTO dbo.PHAN_TICH_CHI_PHI_TMP" & Commons.Modules.UserName & _
                     ...
                     " CHI_PHI_VAT_TU,CHI_PHI_NHAN_CONG,CHI_PHI_DV,CHI_PHI_KHAC,CHI_PHI_HANG_NGAY,TONG) " & _
                     ...
                     vrow("CHI_PHI_KHAC") & "','" & vrow("CHI_PHI_HANG_NGAY") & "','" & vrow("TONG") & "')"
                SqlHelper.ExecuteNonQuery(Commons.IConnections.ConnectionString, CommandType.Text, sqlInsert)
            Next
        End If

    End Sub
    Function DataNX(ByVal MSNX As String) As DataTable
        Dim vtb As New DataTable()
        Dim vsql As String
        vsql = "SELECT SUM( CHI_PHI_PHU_TUNG) AS CHI_PHI_PHU_TUNG ,SUM( CHI_PHI_VAT_TU ) AS CHI_PHI_VAT_TU, " & _
                " SUM( CHI_PHI_NHAN_CONG) AS CHI_PHI_NHAN_CONG,SUM( CHI_PHI_DV) AS CHI_PHI_DV ,SUM( CHI_PHI_KHAC) AS CHI_PHI_KHAC,  " & _
                " SUM(CHI_PHI_HANG_NGAY) AS CHI_PHI_HANG_NGAY, SUM (TONG ) AS TONG   " & _
                " FROM dbo.PHAN_TICH_CHI_PHI_TMP" & Commons.Modules.UserName & _
                " WHERE MS_N_XUONG IN(SELECT MS_N_XUONG FROM [dbo].[MashjGetNXUser]('" & Commons.Modules.UserName & "','" & MSNX & "')) " & _
                " ORDER BY TONG " + vSort
        vtb.Load(SqlHelper.ExecuteReader(Commons.IConnections.ConnectionString, CommandType.Text, vsql))
        Return vtb
        ''
    End Function
    Function DataNhaxuong(ByVal MSNX As String) As DataTable
        Dim vtb As New DataTable()
        Dim vsql As String = "SELECT MS_N_XUONG, TEN_N_XUONG,SUM( CHI_PHI_PHU_TUNG) AS CHI_PHI_PHU_TUNG ,SUM( CHI_PHI_VAT_TU ) AS CHI_PHI_VAT_TU, SUM( CHI_PHI_NHAN_CONG) AS CHI_PHI_NHAN_CONG,SUM( CHI_PHI_DV) AS CHI_PHI_DV ,SUM( CHI_PHI_KHAC) AS CHI_PHI_KHAC, SUM(CHI_PHI_HANG_NGAY) AS CHI_PHI_HANG_NGAY, SUM (TONG ) AS TONG " & _
        " FROM dbo.PHAN_TICH_CHI_PHI_TMP" & Commons.Modules.UserName & _
        " WHERE MS_N_XUONG IN(SELECT MS_N_XUONG FROM [dbo].[MashjGetNXUser]('" & Commons.Modules.UserName & "','" & MSNX & "')) " & _
        " GROUP BY MS_N_XUONG, TEN_N_XUONG  " & _
        " ORDER BY TONG " + vSort
        vtb.Load(SqlHelper.ExecuteReader(Commons.IConnections.ConnectionString, CommandType.Text, vsql))
        Return vtb
        ''
    ...
    Function DataHeThong(ByVal _vNhaXuong As String) As DataTable
        Dim vtb As New DataTable()
        Dim vsql As String = "SELECT MS_HE_THONG, TEN_HE_THONG, SUM(CHI_PHI_PHU_TUNG) AS CHI_PHI_PHU_TUNG, SUM(CHI_PHI_VAT_TU) AS CHI_PHI_VAT_TU, " & _
                     " SUM(CHI_PHI_NHAN_CONG) AS CHI_PHI_NHAN_CONG, SUM(CHI_PHI_DV) AS CHI_PHI_DV, SUM(CHI_PHI_KHAC) AS CHI_PHI_KHAC, " & _
                     " SUM(CHI_PHI_HANG_NGAY) AS CHI_PHI_HANG_NGAY, SUM(TONG) AS TONG " & _
                     " FROM dbo.PHAN_TICH_CHI_PHI_TMP" & Commons.Modules.UserName & _
                     " WHERE MS_N_XUONG IN(SELECT MS_N_XUONG FROM [dbo].[MashjGetNXUser]('" & Commons.Modules.UserName & "','" & _vNhaXuong & "')) " & _
                     " GROUP BY MS_HE_THONG, TEN_HE_THONG" & _
                     " ORDER BY TONG " + vSort
        vtb.Load(SqlHelper.ExecuteReader(Commons.IConnections.ConnectionString, CommandType.Text, vsql))
        Return vtb
    End Function
    ...
    Function DataLoaiMay(ByVal _vNhaXuong As String) As DataTable
        Dim vtb As New DataTable()
        Dim vsql As String = "SELECT     MS_LOAI_MAY, TEN_LOAI_MAY, SUM(CHI_PHI_PHU_TUNG) AS CHI_PHI_PHU_TUNG, SUM(CHI_PHI_VAT_TU) AS CHI_PHI_VAT_TU, " & _
                     " SUM(CHI_PHI_NHAN_CONG) AS CHI_PHI_NHAN_CONG, SUM(CHI_PHI_DV) AS CHI_PHI_DV, SUM(CHI_PHI_KHAC) AS CHI_PHI_KHAC, " & _
                     " SUM(CHI_PHI_HANG_NGAY) AS CHI_PHI_HANG_NGAY, SUM(TONG) AS TONG " & _
                     " FROM dbo.PHAN_TICH_CHI_PHI_TMP" & Commons.Modules.UserName & _
                     " WHERE MS_N_XUONG IN(SELECT MS_N_XUONG FROM [dbo].[MashjGetNXUser]('" & Commons.Modules.UserName & "','" & _vNhaXuong & "')) " & _
                     " GROUP BY MS_LOAI_MAY, TEN_LOAI_MAY" & _
                     " ORDER BY TONG " + vSort
        vtb.Load(SqlHelper.ExecuteReader(Commons.IConnections.ConnectionString, CommandType.Text, vsql))
        Return vtb
    End Function
    ...
    Function DataNhomMayByLoaiMay(ByVal _vNhaXuong As String, ByVal _vLoaiMay As String) As DataTable
        Dim vtb As New DataTable()
        Dim vsql As String = "SELECT  MS_NHOM_MAY, TEN_NHOM_MAY, SUM(CHI_PHI_PHU_TUNG) AS CHI_PHI_PHU_TUNG, SUM(CHI_PHI_VAT_TU) AS CHI_PHI_VAT_TU, " & _
                      " SUM(CHI_PHI_NHAN_CONG) AS CHI_PHI_NHAN_CONG, SUM(CHI_PHI_DV) AS CHI_PHI_DV, SUM(CHI_PHI_KHAC) AS CHI_PHI_KHAC, " & _
                      " SUM(CHI_PHI_HANG_NGAY) AS CHI_PHI_HANG_NGAY, SUM(TONG) AS TONG " & _
                      " FROM dbo.PHAN_TICH_CHI_PHI_TMP" & Commons.Modules.UserName & _
                      " WHERE MS_N_XUONG IN(SELECT MS_N_XUONG FROM [dbo].[MashjGetNXUser]('" & Commons.Modules.UserName & "','" & _vNhaXuong & "')) " & _
                      " AND MS_LOAI_MAY = '" & _vLoaiMay & "'" & _
                      " GROUP BY MS_NHOM_MAY, TEN_NHOM_MAY" & _
                      " ORDER BY TONG " + vSort
        vtb.Load(SqlHelper.ExecuteReader(Commons.IConnections.ConnectionString, CommandType.Text, vsql))
        Return vtb
    End Function
    ...
    Function DataMayByLoaiMay(ByVal _vNhaXuong As String, ByVal _vLoaiMay As String, ByVal _vNhomMay As String) As DataTable
        Dim vtb As New DataTable()
        Dim vsql As String = "SELECT   MS_MAY , SUM(CHI_PHI_PHU_TUNG) AS CHI_PHI_PHU_TUNG, SUM(CHI_PHI_VAT_TU) AS CHI_PHI_VAT_TU, " & _
                     " SUM(CHI_PHI_NHAN_CONG) AS CHI_PHI_NHAN_CONG, SUM(CHI_PHI_DV) AS CHI_PHI_DV, SUM(CHI_PHI_KHAC) AS CHI_PHI_KHAC, " & _
                     " SUM(CHI_PHI_HANG_NGAY) AS CHI_PHI_HANG_NGAY, SUM(TONG) AS TONG " & _
                     " FROM dbo.PHAN_TICH_CHI_PHI_TMP" & Commons.Modules.UserName & _
                     " WHERE MS_N_XUONG IN(SELECT MS_N_XUONG FROM [dbo].[MashjGetNXUser]('" & Commons.Modules.UserName & "','" & _vNhaXuong & "')) " & _
                     " AND MS_LOAI_MAY = '" & _vLoaiMay & "' AND MS_NHOM_MAY = '" & _vNhomMay & "'" & _
                     " GROUP BY MS_MAY" & _
                     " ORDER BY TONG " + vSort
        vtb.Load(SqlHelper.ExecuteReader(Commons.IConnections.ConnectionString, CommandType.Text, vsql))
        Return vtb
    ...
    Function DataMaybyHeThong(ByVal _vNhaXuong As String, ByVal _vHeThong As String, ByVal _vNhomMay As String) As DataTable
        ...
        Dim vsql As String = "SELECT   MS_MAY , SUM(CHI_PHI_PHU_TUNG) AS CHI_PHI_PHU_TUNG, SUM(CHI_PHI_VAT_TU) AS CHI_PHI_VAT_TU, " & _
                     ...
                     " FROM dbo.PHAN_TICH_CHI_PHI_TMP" & Commons.Modules.UserName & _
                     " WHERE MS_N_XUONG IN(SELECT MS_N_XUONG FROM [dbo].[MashjGetNXUser]('" & Commons.Modules.UserName & "','" & _vNhaXuong & "')) " & _
                     " AND MS_HE_THONG = '" & _vHeThong & "' AND MS_NHOM_MAY = '" & _vNhomMay & "'" & _
                     " GROUP BY MS_MAY" & _
                     " ORDER BY TONG " + vSort
        vtb.Load(SqlHelper.ExecuteReader(Commons.IConnections.ConnectionString, CommandType.Text, vsql))
        Return vtb
    ...
    Function DataPBTByLoaiMay(ByVal _vNhaXuong As String, ByVal _vLoaiMay As String, ByVal _vNhomMay As String, ByVal _vMay As String) As DataTable
        Dim vtb As New DataTable()
        Dim vsql As String = "SELECT   MS_PHIEU_BAO_TRI , SUM(CHI_PHI_PHU_TUNG) AS CHI_PHI_PHU_TUNG, SUM(CHI_PHI_VAT_TU) AS CHI_PHI_VAT_TU, " & _
                     " SUM(CHI_PHI_NHAN_CONG) AS CHI_PHI_NHAN_CONG, SUM(CHI_PHI_DV) AS CHI_PHI_DV, SUM(CHI_PHI_KHAC) AS CHI_PHI_KHAC, " & _
                     " SUM(CHI_PHI_HANG_NGAY) AS CHI_PHI_HANG_NGAY, SUM(TONG) AS TONG " & _
                     " FROM dbo.PHAN_TICH_CHI_PHI_TMP" & Commons.Modules.UserName & _
                     " WHERE MS_N_XUONG IN(SELECT MS_N_XUONG FROM [dbo].[MashjGetNXUser]('" & Commons.Modules.UserName & "','" & _vNhaXuong & "')) " & _
                     " AND MS_LOAI_MAY = '" & _vLoaiMay & "' AND MS_NHOM_MAY = '" & _vNhomMay & "' AND MS_MAY=N'" & _vMay & "'" & _
                     " GROUP BY MS_PHIEU_BAO_TRI" & _
                     " ORDER BY TONG " + vSort
        vtb.Load(SqlHelper.ExecuteReader(Commons.IConnections.ConnectionString, CommandType.Text, vsql))
        Return vtb
    End Function
    ...
    Function DataPBTByHeThong(ByVal _vNhaXuong As String, ByVal _vHeThong As String, ByVal _vNhomMay As String, ByVal _vMay As String) As DataTable
        ...
        Dim vsql As String = "SELECT   MS_PHIEU_BAO_TRI , SUM(CHI_PHI_PHU_TUNG) AS CHI_PHI_PHU_TUNG, SUM(CHI_PHI_VAT_TU) AS CHI_PHI_VAT_TU, " & _
                     ...
                     " FROM dbo.PHAN_TICH_CHI_PHI_TMP" & Commons.Modules.UserName & _
                     " WHERE MS_N_XUONG IN(SELECT MS_N_XUONG FROM [dbo].[MashjGetNXUser]('" & Commons.Modules.UserName & "','" & _vNhaXuong & "')) " & _
                     " AND MS_HE_THONG = '" & _vHeThong & "' AND MS_NHOM_MAY = '" & _vNhomMay & "' AND MS_MAY=N'" & _vMay & "'" & _
                     " GROUP BY MS_PHIEU_BAO_TRI" & _
                     " ORDER BY TONG " + vSort
        vtb.Load(SqlHelper.ExecuteReader(Commons.IConnections.ConnectionString, CommandType.Text, vsql))
        Return vtb
    ...
    Private Sub ViewData()
        If KiemTraData() Then
            ...
            TSource.Load(SqlHelper.ExecuteReader(Commons.IConnections.ConnectionString, CommandType.Text, _
                " SELECT * , ( SELECT SUM(TONG) FROM PHAN_TICH_CHI_PHI_TMP" & Commons.Modules.UserName & " A " & _
                    " WHERE A.MS_N_XUONG  IN (SELECT MS_N_XUONG FROM [dbo].[MashjGetNXUser]('-1',B.MS_N_XUONG))) AS TONG " & _
                    " FROM dbo.NHA_XUONG B ORDER BY TONG " + vSort + " , TEN_N_XUONG"))

            TSource.DefaultView.RowFilter = "MS_CHA is null or MS_CHA = ''"
            Dim TParent As DataTable = TSource.DefaultView.ToTable()

            Dim MS, Ten As String


            For Each Row In TParent.Rows

                MS = Row("MS_N_XUONG").ToString().Trim()
                If Row("GHI_CHU").ToString().Trim() = "" Then
                    Ten = Row("TEN_N_XUONG").ToString().Trim()
                Else
                    Ten = Row("TEN_N_XUONG").ToString().Trim() + "(" + Row("GHI_CHU").ToString().Trim() + ")"
                End If
                ...
                If vtbNx.Rows.Count > 0 Then
                    For Each vRowNx As DataRow In vtbNx.Rows
                        node = gvData.Nodes.Add(Nothing, Ten, vRowNx("CHI_PHI_PHU_TUNG"), vRowNx("CHI_PHI_VAT_TU"), vRowNx("CHI_PHI_NHAN_CONG"), vRowNx("CHI_PHI_DV"), vRowNx("CHI_PHI_KHAC"), vRowNx("CHI_PHI_HANG_NGAY"), vRowNx("TONG"))
                    Next
                Else
                    node = gvData.Nodes.Add(Nothing, Ten, Nothing, Nothing, Nothing, Nothing, Nothing, Nothing, Nothing)
                End If
                node.DefaultCellStyle.ForeColor = Color.Blue
                node.ImageIndex = 0
                TaoTree(MS, node)

                CreateTreeNode(MS, node)
            Next
    ...
    Private Sub TaoTree(ByVal MsNX As String, ByVal node As TreeGridNode)
        Dim vtbNx As New DataTable()
        vtbNx = DataNhaxuong(MsNX)
        For Each vRowNx As DataRow In vtbNx.Rows
            'Dim node As TreeGridNode = gvData.Nodes.Add(Nothing, vRowNx("TEN_N_XUONG").ToString, vRowNx("CHI_PHI_PHU_TUNG"), vRowNx("CHI_PHI_VAT_TU"), vRowNx("CHI_PHI_NHAN_CONG"), vRowNx("CHI_PHI_DV"), vRowNx("CHI_PHI_KHAC"), vRowNx("CHI_PHI_HANG_NGAY"), vRowNx("TONG"))
            'node = gvData.Nodes.Add(Nothing, vRowNx("TEN_N_XUONG").ToString, vRowNx("CHI_PHI_PHU_TUNG"), vRowNx("CHI_PHI_VAT_TU"), vRowNx("CHI_PHI_NHAN_CONG"), vRowNx("CHI_PHI_DV"), vRowNx("CHI_PHI_KHAC"), vRowNx("CHI_PHI_HANG_NGAY"), vRowNx("TONG"))
            'node = node.Parent.Nodes.Add(Nothing, vRowNx("TEN_N_XUONG").ToString, vRowNx("CHI_PHI_PHU_TUNG"), vRowNx("CHI_PHI_VAT_TU"), vRowNx("CHI_PHI_NHAN_CONG"), vRowNx("CHI_PHI_DV"), vRowNx("CHI_PHI_KHAC"), vRowNx("CHI_PHI_HANG_NGAY"), vRowNx("TONG"))
            node.ImageIndex = 0
            'Loai may
            'node.DefaultCellStyle.Font = boldFont
            node.DefaultCellStyle.ForeColor = Color.Blue
            node = node.Nodes.Add(Nothing, "Loại máy", Nothing, Nothing, Nothing, Nothing, Nothing, Nothing, Nothing)
            ...
            For Each vRowLm As DataRow In vtbLoaiMay.Rows
                If c3 = 0 Then
                    node = node.Nodes.Add(Nothing, vRowLm("TEN_LOAI_MAY").ToString, vRowLm("CHI_PHI_PHU_TUNG"), vRowLm("CHI_PHI_VAT_TU"), vRowLm("CHI_PHI_NHAN_CONG"), vRowLm("CHI_PHI_DV"), vRowLm("CHI_PHI_KHAC"), vRowLm("CHI_PHI_HANG_NGAY"), vRowLm("TONG"))
                    node.ImageIndex = 0
                    nodeCap3 = node
                    c3 = c3 + 1
                    'Nhóm Máy
                    ...
                    For Each vRowNm As DataRow In vtbNhomMay.Rows
                        If c4 = 0 Then
                            'node.DefaultCellStyle.Font = boldFont
                            node.DefaultCellStyle.ForeColor = Color.Maroon
                            node = node.Nodes.Add(Nothing, vRowNm("TEN_NHOM_MAY").ToString, vRowNm("CHI_PHI_PHU_TUNG"), vRowNm("CHI_PHI_VAT_TU"), vRowNm("CHI_PHI_NHAN_CONG"), vRowNm("CHI_PHI_DV"), vRowNm("CHI_PHI_KHAC"), vRowNm("CHI_PHI_HANG_NGAY"), vRowNm("TONG"))
                            node.ImageIndex = 0
                            nodeCap4 = node
                            c4 = c4 + 1
                            'Máy
                            ...
                            For Each vRowMay As DataRow In vtbMay.Rows
                                If c5 = 0 Then
                                    ...
                                    node = node.Nodes.Add(Nothing, vRowMay("MS_MAY").ToString, vRowMay("CHI_PHI_PHU_TUNG"), vRowMay("CHI_PHI_VAT_TU"), vRowMay("CHI_PHI_NHAN_CONG"), vRowMay("CHI_PHI_DV"), vRowMay("CHI_PHI_KHAC"), vRowMay("CHI_PHI_HANG_NGAY"), vRowMay("TONG"))
                                    node.ImageIndex = 0
                                    nodeCap5 = node
                                    c5 = c5 + 1
                                    'Phieu Bao Tri    
                                    ...
                                    For Each vRowPBT As DataRow In vtbPhieuBaotri.Rows
                                        Try
                                            ...
                                            If c6 = 0 Then
                                                node = node.Nodes.Add(Nothing, vRowPBT("MS_PHIEU_BAO_TRI").ToString, vRowPBT("CHI_PHI_PHU_TUNG"), vRowPBT("CHI_PHI_VAT_TU"), vRowPBT("CHI_PHI_NHAN_CONG"), vRowPBT("CHI_PHI_DV"), vRowPBT("CHI_PHI_KHAC"), vRowPBT("CHI_PHI_HANG_NGAY"), vRowPBT("TONG"))
                                                node.ImageIndex = 0
                                                nodeCap6 = node
                                                c6 = c6 + 1
                                            Else
                                                node = nodeCap6
                                                node = node.Parent.Nodes.Add(Nothing, vRowPBT("MS_PHIEU_BAO_TRI").ToString, vRowPBT("CHI_PHI_PHU_TUNG"), vRowPBT("CHI_PHI_VAT_TU"), vRowPBT("CHI_PHI_NHAN_CONG"), vRowPBT("CHI_PHI_DV"), vRowPBT("CHI_PHI_KHAC"), vRowPBT("CHI_PHI_HANG_NGAY"), vRowPBT("TONG"))
                                                node.ImageIndex = 0
                                            End If
                                        Catch ex As Exception
                                ...
                                Else
                                    ...
                                    'node.DefaultCellStyle.Font = boldFont
                                    'node.DefaultCellStyle.ForeColor = Color.Green
                                    node = node.Parent.Nodes.Add(Nothing, vRowMay("MS_MAY").ToString, vRowMay("CHI_PHI_PHU_TUNG"), vRowMay("CHI_PHI_VAT_TU"), vRowMay("CHI_PHI_NHAN_CONG"), vRowMay("CHI_PHI_DV"), vRowMay("CHI_PHI_KHAC"), vRowMay("CHI_PHI_HANG_NGAY"), vRowMay("TONG"))
                                    node.ImageIndex = 0
                                    'Phieu Bao Tri    
                                    Dim vtbPhieuBaotri As New DataTable
                                    vtbPhieuBaotri = DataPBTByLoaiMay(vRowNx("MS_N_XUONG").ToString, vRowLm("MS_LOAI_MAY").ToString, vRowNm("MS_NHOM_MAY").ToString, vRowMay("MS_MAY").ToString)
                        ...
                        Else
                            ...
                            node = node.Parent.Nodes.Add(Nothing, vRowNm("TEN_NHOM_MAY").ToString, vRowNm("CHI_PHI_PHU_TUNG"), vRowNm("CHI_PHI_VAT_TU"), vRowNm("CHI_PHI_NHAN_CONG"), vRowNm("CHI_PHI_DV"), vRowNm("CHI_PHI_KHAC"), vRowNm("CHI_PHI_HANG_NGAY"), vRowNm("TONG"))
                            node.ImageIndex = 0
                            Dim vtbMay As New DataTable
                            vtbMay = DataMayByLoaiMay(vRowNx("MS_N_XUONG").ToString, vRowLm("MS_LOAI_MAY").ToString, vRowNm("MS_NHOM_MAY").ToString)
                            Dim c5 As Integer = 0
                ...
                Else
                    node = nodeCap3
                    node = node.Parent.Nodes.Add(Nothing, vRowLm("TEN_LOAI_MAY").ToString, vRowLm("CHI_PHI_PHU_TUNG"), vRowLm("CHI_PHI_VAT_TU"), vRowLm("CHI_PHI_NHAN_CONG"), vRowLm("CHI_PHI_DV"), vRowLm("CHI_PHI_KHAC"), vRowLm("CHI_PHI_HANG_NGAY"), vRowLm("TONG"))
                    node.ImageIndex = 0
                    Dim vtbNhomMay As New DataTable()
                    vtbNhomMay = DataNhomMayByLoaiMay(vRowNx("MS_N_XUONG").ToString, vRowLm("MS_LOAI_MAY").ToString)
                    Dim c4 As Integer = 0
            ...
            node = nodeCap2
            'node.DefaultCellStyle.Font = boldFont
            node = node.Parent.Nodes.Add(Nothing, "Dây chuyền", Nothing, Nothing, Nothing, Nothing, Nothing, Nothing, Nothing)
            ...
            For Each vRowLm As DataRow In vtbHeThong.Rows
                If c31 = 0 Then
                    node = node.Nodes.Add(Nothing, vRowLm("TEN_HE_THONG").ToString, vRowLm("CHI_PHI_PHU_TUNG"), vRowLm("CHI_PHI_VAT_TU"), vRowLm("CHI_PHI_NHAN_CONG"), vRowLm("CHI_PHI_DV"), vRowLm("CHI_PHI_KHAC"), vRowLm("CHI_PHI_HANG_NGAY"), vRowLm("TONG"))
                    node.ImageIndex = 2
                    nodeCap3 = node
                    c31 = c31 + 1
                    'Nhóm Máy
                ...
                Else
                    node = nodeCap3
                    node = node.Parent.Nodes.Add(Nothing, vRowLm("TEN_HE_THONG").ToString, vRowLm("CHI_PHI_PHU_TUNG"), vRowLm("CHI_PHI_VAT_TU"), vRowLm("CHI_PHI_NHAN_CONG"), vRowLm("CHI_PHI_DV"), vRowLm("CHI_PHI_KHAC"), vRowLm("CHI_PHI_HANG_NGAY"), vRowLm("TONG"))
                    node.ImageIndex = 2
                    Dim vtbNhomMay As New DataTable()
                    vtbNhomMay = DataNhomMayByHeThong(vRowNx("MS_N_XUONG").ToString, vRowLm("MS_HE_THONG").ToString)
                    Dim c4 As Integer = 0
            ...
            node = nodeCap2
            'node.DefaultCellStyle.Font = boldFont
            node = node.Parent.Nodes.Add(Nothing, "Máy", Nothing, Nothing, Nothing, Nothing, Nothing, Nothing, Nothing)
            node.DefaultCellStyle.ForeColor = Color.Chocolate
            node.ImageIndex = 1

            Dim nodeCap21 As TreeGridNode
            nodeCap21 = node
            Dim sql As String
            sql = "SELECT   MS_MAY , SUM(CHI_PHI_PHU_TUNG) AS CHI_PHI_PHU_TUNG, SUM(CHI_PHI_VAT_TU) AS CHI_PHI_VAT_TU, " & _
                     " SUM(CHI_PHI_NHAN_CONG) AS CHI_PHI_NHAN_CONG, SUM(CHI_PHI_DV) AS CHI_PHI_DV, SUM(CHI_PHI_KHAC) AS CHI_PHI_KHAC, " & _
                     " SUM(CHI_PHI_HANG_NGAY) AS CHI_PHI_HANG_NGAY, SUM(TONG) AS TONG " & _
                     " FROM dbo.PHAN_TICH_CHI_PHI_TMP" & Commons.Modules.UserName & _
                     " WHERE MS_N_XUONG IN(SELECT MS_N_XUONG FROM [dbo].[MashjGetNXUser]('" & Commons.Modules.UserName & "','" & vRowNx("MS_N_XUONG").ToString & "')) " & _
                     " GROUP BY MS_MAY ORDER BY TONG " + vSort


            Dim vtbMay1 As DataTable = New DataTable
            vtbMay1.Load(SqlHelper.ExecuteReader(Commons.IConnections.ConnectionString, CommandType.Text, sql))
            Dim c51 As Integer = 0
            Dim nodeCap51 As TreeGridNode = Nothing
            For Each vRowMay As DataRow In vtbMay1.Rows
                If c51 = 0 Then
                    'node.DefaultCellStyle.Font = boldFont
                    'node.DefaultCellStyle.ForeColor = Color.Brown
                    node = node.Nodes.Add(Nothing, vRowMay("MS_MAY").ToString, vRowMay("CHI_PHI_PHU_TUNG"), vRowMay("CHI_PHI_VAT_TU"), vRowMay("CHI_PHI_NHAN_CONG"), vRowMay("CHI_PHI_DV"), vRowMay("CHI_PHI_KHAC"), vRowMay("CHI_PHI_HANG_NGAY"), vRowMay("TONG"))
                    node.DefaultCellStyle.ForeColor = Color.Chocolate
                    node.ImageIndex = 1
                    nodeCap51 = node
                    ...
                    Dim vsql As String = "SELECT   MS_PHIEU_BAO_TRI , SUM(CHI_PHI_PHU_TUNG) AS CHI_PHI_PHU_TUNG, SUM(CHI_PHI_VAT_TU) AS CHI_PHI_VAT_TU, " & _
                                         ...
                                         " FROM dbo.PHAN_TICH_CHI_PHI_TMP" & Commons.Modules.UserName & _
                                         " WHERE MS_N_XUONG IN(SELECT MS_N_XUONG FROM [dbo].[MashjGetNXUser]('" & Commons.Modules.UserName & "','" & vRowNx("MS_N_XUONG").ToString & "')) " & _
                                         " AND MS_MAY=N'" & vRowMay("MS_MAY").ToString & "'" & _
                                         " GROUP BY MS_PHIEU_BAO_TRI" & _
                                         " ORDER BY TONG " + vSort
                    vtbPhieuBaotri.Load(SqlHelper.ExecuteReader(Commons.IConnections.ConnectionString, CommandType.Text, vsql))

                    Dim c6 As Integer = 0
                    Dim nodeCap61 As TreeGridNode = Nothing
                    For Each vRowPBT As DataRow In vtbPhieuBaotri.Rows
                        If c6 = 0 Then
                            node = node.Nodes.Add(Nothing, vRowPBT("MS_PHIEU_BAO_TRI").ToString, vRowPBT("CHI_PHI_PHU_TUNG"), vRowPBT("CHI_PHI_VAT_TU"), vRowPBT("CHI_PHI_NHAN_CONG"), vRowPBT("CHI_PHI_DV"), vRowPBT("CHI_PHI_KHAC"), vRowPBT("CHI_PHI_HANG_NGAY"), vRowPBT("TONG"))
                            node.ImageIndex = 1
                ...
                Try
                    ...
                    Try
                        ...
                        If iTinhtrangphieu >= 0 Then
                            ...
                            frmRpt.ShowDialog()
                        Else
                            MsgBox(Commons.Modules.ObjLanguages.GetLanguage(Commons.Modules.ModuleName, "rptPhanTichChiPhi", "Phieubaotrichuacapnhattinhtrang", Commons.Modules.TypeLanguage))
                        End If
                    Catch ex As Exception
                    End Try
                Catch ex As Exception
    ...
    Private Sub CreateTreeNode(ByVal MSCha As String, ByVal nParent1 As TreeGridNode)
        Try

            Dim MS, Ten As String

            TSource.DefaultView.RowFilter = "MS_CHA = '" + MSCha + "'"
            Dim TParent As DataTable = TSource.DefaultView.ToTable()
            Dim i As Integer
            i = 0
            ...
            For Each Row In TParent.Rows
                ...
                If vtbNx.Rows.Count > 0 Then
                    For Each vRowNx As DataRow In vtbNx.Rows
                        If i = 0 Then
                            nParent1 = nParent1.Nodes.Add(Nothing, Ten, vRowNx("CHI_PHI_PHU_TUNG"), vRowNx("CHI_PHI_VAT_TU"), vRowNx("CHI_PHI_NHAN_CONG"), vRowNx("CHI_PHI_DV"), vRowNx("CHI_PHI_KHAC"), vRowNx("CHI_PHI_HANG_NGAY"), vRowNx("TONG"))
                        Else
                            nParent1 = nParent1.Parent.Nodes.Add(Nothing, Ten, vRowNx("CHI_PHI_PHU_TUNG"), vRowNx("CHI_PHI_VAT_TU"), vRowNx("CHI_PHI_NHAN_CONG"), vRowNx("CHI_PHI_DV"), vRowNx("CHI_PHI_KHAC"), vRowNx("CHI_PHI_HANG_NGAY"), vRowNx("TONG"))
                        End If
                    Next
                Else
                    If i = 0 Then
                        nParent1 = nParent1.Nodes.Add(Nothing, Ten, Nothing, Nothing, Nothing, Nothing, Nothing, Nothing, Nothing)
                    Else
                        nParent1 = nParent1.Parent.Nodes.Add(Nothing, Ten, Nothing, Nothing, Nothing, Nothing, Nothing, Nothing, Nothing)
                    End If
                End If
                nParent1.DefaultCellStyle.ForeColor = Color.Blue
                nParent1.ImageIndex = 0
                TaoTree(MS, nParent1)
                i = i + 1
                CreateTreeNode(MS, nParent1)
            Next
        Catch ex As Exception
    ...
    Private Sub btnLoad_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnLoad.Click
        Try
            If mtxtTuNgay.Text.Trim <> "/  /" Then
                Try
                    vTuNgay = DateTime.ParseExact(mtxtTuNgay.Text.Trim, "dd/MM/yyyy", System.Globalization.CultureInfo.CurrentCulture)
                Catch ex As Exception
                    gvData.Nodes.Clear()
                    MsgBox(Commons.Modules.ObjLanguages.GetLanguage(Commons.Modules.ModuleName, "rptPhanTichChiPhi", "CHUA_DUNG_DINH_DANG", Commons.Modules.TypeLanguage))
                    Exit Sub
                End Try
            End If
        Catch ex As Exception
        ...
        Try
            If mtxtDenNgay.Text.Trim <> "/  /" Then
                ...
                If vDenNgay < vTuNgay Then
                    MsgBox(Commons.Modules.ObjLanguages.GetLanguage(Commons.Modules.ModuleName, "rptPhanTichChiPhi", "DN_PHAI_LON_HON_TN", Commons.Modules.TypeLanguage))
                    Exit Sub
                End If
            End If
        Catch ex As Exception
