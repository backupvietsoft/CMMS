...
Public Class frmrptHIEU_SUAT_SU_DUNG_MAY

    Private Sub frmrptHIEU_SUAT_SU_DUNG_MAY_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Try
    ...
    Private Sub btnThucHien_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnThucHien.Click
        Try
            ...
            With ExcelSheets
                ...
                .Cells(vRow, 1) = "HIỆU SUẤT SỬ DỤNG MÁY THEO DÂY CHYỀN"
                vRow = vRow + 1
            ...
            For j As Integer = 0 To gvDaychuyen.Rows.Count - 1
                If (Not gvDaychuyen.Rows(j).Cells("clChonDaychuyen").Value Is Nothing) Then
                    If Convert.ToBoolean(gvDaychuyen.Rows(j).Cells("clChonDaychuyen").Value) Then
                        ...
                        " AND MS_HE_THONG = " & gvDaychuyen.Rows(j).Cells("clMsDaychuyen").Value & " AND THOI_GIAN_SUA_CHUA > 0 AND (BTDK = 1 OR HU_HONG = 1 OR (BTDK = 0 AND HU_HONG = 0 ) OR (BTDK IS NULL AND HU_HONG IS NULL )) "
                        ...
                        With ExcelSheets
                            ...
                            For k As Integer = 0 To vTablemay.Rows.Count - 1
                                ...
                                While vNgay.Year <= vDen.Year
                                    ...
                                    Else
                                        ...
                                        " WHERE NGAY >= '" & vTu.ToString("MM/dd/yyyy") & "'  AND NGAY < '" & vDen.ToString("MM/dd/yyyy") & "'AND YEAR (THANG) = YEAR (NGAY) AND MONTH (THANG) = MONTH (NGAY) AND ((BTDK = 1 OR HU_HONG = 1 OR (BTDK = 0 AND HU_HONG = 0 ) OR (BTDK IS NULL AND HU_HONG IS NULL ) OR (BTDK IS NULL AND HU_HONG =0 )OR (BTDK = 0 AND HU_HONG IS NULL )))And Month(NGAY) = " & vNgay.Month & " AND YEAR(NGAY) = " & vNgay.Year & " " & _
                                        ...
                                        Try
                                            .Cells(vRow, vColum).BorderAround(ColorIndex:=1, Weight:=Excel.XlBorderWeight.xlThin)
                                            .Cells(vRow, vColum) = Format(Convert.ToDouble(tbTb.Rows(0)("KHA_NANG_SAN_SANG")), "N4").ToString()
                                        Catch ex As Exception
                                        End Try
                                        vColum = vColum + 1
                                        vNgay = vNgay.AddMonths(1)
                            ...
                            If vTablemay.Rows.Count > 0 Then
                                ...
                                While vNgay.Year <= vDen.Year
                                    ...
                                    If (vColum > 2) Then
                                        Dim tbHS As DataTable = New DataTable()
                                        Dim strsshs As String = "SELECT ((dbo.KE_HOACH_SX_LINE.SO_GIO_KH  - SUM ( dbo.THOI_GIAN_NGUNG_MAY.THOI_GIAN_SUA_CHUA/60)) / dbo.KE_HOACH_SX_LINE.SO_GIO_KH ) AS KHA_NANG_SAN_SANG" & _
                                        " FROM         dbo.KE_HOACH_SX_LINE INNER JOIN" & _
                                        " dbo.HE_THONG ON dbo.KE_HOACH_SX_LINE.MS_HE_THONG = dbo.HE_THONG.MS_HE_THONG INNER JOIN " & _
                                        " dbo.THOI_GIAN_NGUNG_MAY ON dbo.HE_THONG.MS_HE_THONG = dbo.THOI_GIAN_NGUNG_MAY.MS_HE_THONG INNER JOIN " & _
                                        " dbo.NGUYEN_NHAN_DUNG_MAY ON dbo.THOI_GIAN_NGUNG_MAY.MS_NGUYEN_NHAN = dbo.NGUYEN_NHAN_DUNG_MAY.MS_NGUYEN_NHAN " & _
                                        ...
                                        "GROUP BY dbo.KE_HOACH_SX_LINE.SO_GIO_KH "
                                        tbHS.Load(SqlHelper.ExecuteReader(Commons.IConnections.ConnectionString, CommandType.Text, strsshs))
                                        Try
                                            .Cells(vRow, vColum) = Format(Convert.ToDouble(tbHS.Rows(0)("KHA_NANG_SAN_SANG")), "N4").ToString()
                                        Catch ex As Exception
                                        End Try
                                        '.Cells(vRow, vColum) = ""
                                    End If
