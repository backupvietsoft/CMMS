...
Namespace VS.Classes.Catalogue
    Public Class IC_DON_HANG_XUAT_Controller
        ...
        ''' <summary>
        ''' Store: QL_LOAD_LIST_DH_XUAT_VAT_TU_CHI_TIET - QL_LOAD_VI_TRI_KHO_VAT_TU
        ''' </summary>
        ''' <param name="MS_KHO">string</param>
        ''' <param name="MS_DH_XUAT_PT">string</param>
        ''' <param name="MS_PT">string</param>
        ''' <returns></returns>
        Public Function Load_Don_Hang_Xuat_Vat_Tu_Chi_Tiet(ByVal MS_KHO As Integer, ByVal MS_DH_XUAT_PT As String, ByVal MS_PT As String) As List(Of CHI_TIET_VAT_TU_XUAT_Info)
            ...
            If lstDHXVTCT IsNot Nothing Then
                ...
                For i As Integer = 0 To lstDHXVTCT.Count - 1
                    dtReader = SqlHelper.ExecuteReader(connectionString, "QL_LOAD_VI_TRI_KHO_VAT_TU", MS_KHO,
                        lstDHXVTCT.Item(i).MS_PT, lstDHXVTCT.Item(i).MS_VI_TRI,
        ...
        Public Function LOAD_LIST_DON_HANG_XUAT_TU_UNLOCK_KHAC_PHIEU_XUAT_CHON(ByVal MS_DH_XUAT_PT As String, ByVal MS_KHO As Integer) As List(Of IC_DON_HANG_XUAT_VAT_TU_Info)
            ...
            Try
                lstDHXVATTU = QLBusinessDataController.FillCollection(Of IC_DON_HANG_XUAT_VAT_TU_Info)(SqlHelper.ExecuteReader(connectionString,
        ...
        Public Function ADD_DON_HANG_XUAT_CS(ByVal objDonHangXuat As IC_DON_HANG_XUAT_Info, ByVal lstDHXVT As List(Of IC_DON_HANG_XUAT_VAT_TU_Info), ByVal lstCTVTPT As List(Of CHI_TIET_VAT_TU_XUAT_Info)) As Boolean
            Dim objConnection As New SqlConnection(connectionString)
            Dim sl_ton As Decimal? = 0
            objConnection.Open()
            Dim objTrans As SqlTransaction = objConnection.BeginTransaction
            Try
                ...
                'ADD DON HANG XUAT
                SqlHelper.ExecuteNonQuery(objTrans, "H_QL_ADD_DON_HANG_XUAT_CS", objDonHangXuat.MS_DH_XUAT_PT,
                    objDonHangXuat.SO_PHIEU_XN, objDonHangXuat.GIO, objDonHangXuat.NGAY, objDonHangXuat.MS_KHO,
                    objDonHangXuat.MS_DANG_XUAT, objDonHangXuat.NGUOI_NHAN, objDonHangXuat.NGAY_CHUNG_TU,
                    objDonHangXuat.SO_CHUNG_TU, objDonHangXuat.MS_PHIEU_BAO_TRI, objDonHangXuat.GHI_CHU,
                    objDonHangXuat.LOCK, objDonHangXuat.LY_DO_XUAT, objDonHangXuat.THU_KHO,
                    IIf(objDonHangXuat.MS_BP_CHIU_PHI <> 0, objDonHangXuat.MS_BP_CHIU_PHI, DBNull.Value),
                    objDonHangXuat.NGUOI_LAP, objDonHangXuat.CAN_CU, objDonHangXuat.THU_KHO_KY, objDonHangXuat.MS_KHO_DEN, objDonHangXuat.MS_VT_DEN,
                ...
                For i As Integer = 0 To lstDHXVT.Count - 1
                    ...
                    SqlHelper.ExecuteNonQuery(objTrans, "QL_ADD_DON_HANG_XUAT_VAT_TU", objDHXVT.MS_DH_XUAT_PT, objDHXVT.MS_PT,
                            objDHXVT.SO_LUONG_CTU, objDHXVT.SO_LUONG_THUC_XUAT, objDHXVT.GHI_CHU, objDHXVT.TG_PB, objDHXVT.CHI_PHI)
                    If lstCTVTPT.Count > 0 Then
                        Dim lstDHX_VTCT As List(Of CHI_TIET_VAT_TU_XUAT_Info) = lstCTVTPT.Where(Function(x) x.MS_PT.Equals(objDHXVT.MS_PT)).ToList()
                        For j As Integer = 0 To lstDHX_VTCT.Count - 1
                            Dim objDHX_VTCT As CHI_TIET_VAT_TU_XUAT_Info = lstDHX_VTCT.Item(j)
                            If objDHX_VTCT.SL_XUAT <> 0 Then
                                ...
                                SqlHelper.ExecuteNonQuery(objTrans, "QL_ADD_DON_HANG_XUAT_VAT_TU_CHI_TIET", objDHXVTCT.MS_DH_XUAT_PT,
                                    objDHXVTCT.MS_PT, objDHXVTCT.STT, objDHXVTCT.MS_DH_NHAP_PT, objDHXVTCT.MS_VI_TRI,
                                ...
                                sl_ton = SqlHelper.ExecuteScalar(objTrans, "SP_NHU_Y_GET_SO_LUONG_TON_OLD",
                                    objDHXVTCT.MS_DH_NHAP_PT, objDHXVTCT.MS_VI_TRI, objDHXVTCT.MS_PT,
                                    objDHXVTCT.ID_XUAT)
                                objVTKHO_VTU.SL_VT = sl_ton - objDHX_VTCT.SL_XUAT
                                If objVTKHO_VTU.SL_VT < 0 Then
                                    objTrans.Rollback()
                                    XtraMessageBox.Show(Commons.Modules.ObjLanguages.GetLanguage(Commons.Modules.ModuleName, "FrmPhieuXuatKhoVatTu_CS", "msgKhongDuTon", Commons.Modules.TypeLanguage), "", Windows.Forms.MessageBoxButtons.OK, Windows.Forms.MessageBoxIcon.Warning)
                                    Return False
                                End If
                                SqlHelper.ExecuteNonQuery(objTrans, "QL_UPDATE_VI_TRI_KHO_VAT_TU",
                                    objVTKHO_VTU.MS_KHO, objVTKHO_VTU.MS_PT, objVTKHO_VTU.MS_VI_TRI,
                ...
                If objDonHangXuat.MS_DANG_XUAT = 9 Then
                    ...
                    Dim dt As DataTable =  Commons.Modules.ObjSystems.ConvertToDataTable(lstCTVTPT.Select(Function(x) new with{x.MS_DH_NHAP_PT, x.MS_PT,x.DON_GIA,x.MS_VI_TRI,x.ID_XUAT}).ToList())
                    Commons.Modules.ObjSystems.MCreateTableToDatatable(Commons.IConnections.CNStr,"sbtViTriVT"+Commons.Modules.UserName,dt,"")
                    'objTrans.Commit()
                    SqlHelper.ExecuteNonQuery(objTrans, "AddDonhangNhapTuDonHangXuat", objDonHangXuat.MS_DH_XUAT_PT, _MS_DH_NHAP_PT_AUTO, objDonHangXuat.MS_VT_DEN,"sbtViTriVT"+Commons.Modules.UserName)
                    ...
                    If dttmp.Rows.Count > 0 Then
                        XtraMessageBox.Show(dttmp.Rows(0)(3).ToString() + vbCrLf + dttmp.Rows(0)(5).ToString())
                        objTrans.Rollback()
                        XtraMessageBox.Show(Commons.Modules.ObjLanguages.GetLanguage(Commons.Modules.ModuleName, "FrmPhieuXuatKhoVatTu_CS", "msgXuatChuyenKhoKhongThanhCong", Commons.Modules.TypeLanguage), "", Windows.Forms.MessageBoxButtons.OK, Windows.Forms.MessageBoxIcon.Warning)
                        Return False
                    End If
                End If
                If objDonHangXuat.MS_DANG_XUAT = 10 Then
                    Dim _MS_DH_NHAP_PT_AUTO As String = ""
                    _MS_DH_NHAP_PT_AUTO = OSystems.VSGetID("NTL", objDonHangXuat.NGAY)
                    SqlHelper.ExecuteNonQuery(objTrans, "AddDonhangNhapThanhLyTuDonHangXuat", objDonHangXuat.MS_DH_XUAT_PT, _MS_DH_NHAP_PT_AUTO, objDonHangXuat.MS_VT_DEN, Commons.Modules.UserName)
                    Dim dttmp = New DataTable
                    If Commons.Modules.iTRFData = 1 Then dttmp.Load(SqlHelper.ExecuteReader(objTrans, "spTRFNhapKho", _MS_DH_NHAP_PT_AUTO, -1))
                    If dttmp.Rows.Count > 0 Then
                        XtraMessageBox.Show(dttmp.Rows(0)(3).ToString() + vbCrLf + dttmp.Rows(0)(5).ToString())
                ...
                If objDonHangXuat.MS_DANG_XUAT = 8 And objDonHangXuat.MS_MAY <> "" Then
                    Try
                        ...
                        Dim Str As String = " SELECT    SUM(ISNULL(A.SL_VT,0) *  ISNULL(C.DON_GIA,0) *  ISNULL(C.TY_GIA,0)) AS TT " &
                                    " FROM dbo.IC_DON_HANG_XUAT_VAT_TU_CHI_TIET AS A INNER JOIN dbo.IC_DON_HANG_NHAP_VAT_TU_CHI_TIET AS B ON  " &
                                    " A.MS_DH_NHAP_PT = B.MS_DH_NHAP_PT AND A.ID_XUAT = B.ID AND A.MS_VI_TRI = B.MS_VI_TRI AND A.MS_PT = B.MS_PT INNER JOIN " &
                                    " dbo.IC_DON_HANG_NHAP_VAT_TU AS C ON B.MS_DH_NHAP_PT = C.MS_DH_NHAP_PT AND B.MS_PT = C.MS_PT AND B.ID = C.ID " &
                        ...
                        Str = " INSERT INTO CONG_VIEC_HANG_NGAY_VT(STT_CV,MS_PT, MS_DH_XUAT_PT, MS_DH_NHAP_PT,SO_LUONG,ID) " &
                                    " SELECT  " & STT.ToString & "  ,  MS_PT, MS_DH_XUAT_PT, MS_DH_NHAP_PT, SUM(SL_VT) AS SL_VT, ID_XUAT " &
                                    " FROM dbo.IC_DON_HANG_XUAT_VAT_TU_CHI_TIET " &
                                    " WHERE MS_DH_XUAT_PT = '" & objDonHangXuat.MS_DH_XUAT_PT & "' " &
                                    " GROUP BY MS_PT, MS_DH_XUAT_PT, MS_DH_NHAP_PT, ID_XUAT "
                        SqlHelper.ExecuteNonQuery(objTrans, CommandType.Text, Str)

                        Str = " INSERT INTO CONG_VIEC_HANG_NGAY_THIET_BI_VT_CT " &
                                    " (STT_CV, MS_MAY, MS_PT, SO_LUONG, DON_GIA, THANH_TIEN, MS_DH_XUAT_PT, MS_DH_NHAP_PT, ID) " &
                                    " SELECT  " & STT.ToString & " , '" & objDonHangXuat.MS_MAY & "' ,A.MS_PT,A.SL_VT,C.DON_GIA,  " &
                                    " ISNULL(A.SL_VT, 0) * ISNULL(C.DON_GIA, 0) * ISNULL(C.TY_GIA, 0) AS TT, A.MS_DH_XUAT_PT, A.MS_DH_NHAP_PT, A.ID_XUAT " &
                                    " FROM         dbo.IC_DON_HANG_XUAT_VAT_TU_CHI_TIET AS A INNER JOIN " &
                                    " dbo.IC_DON_HANG_NHAP_VAT_TU_CHI_TIET AS B ON A.MS_DH_NHAP_PT = B.MS_DH_NHAP_PT AND A.ID_XUAT = B.ID  " &
                                    " AND A.MS_VI_TRI = B.MS_VI_TRI AND A.MS_PT = B.MS_PT INNER JOIN " &
                                    " dbo.IC_DON_HANG_NHAP_VAT_TU AS C ON B.MS_DH_NHAP_PT = C.MS_DH_NHAP_PT AND B.MS_PT = C.MS_PT AND B.ID = C.ID " &
                ...
                If Commons.Modules.iTRFData = 1 Then
                    If TRFNhapKho(objTrans, objDonHangXuat.MS_DH_XUAT_PT, -1) = False Then
                        objTrans.Rollback()
                        XtraMessageBox.Show(Commons.Modules.ObjLanguages.GetLanguage(Commons.Modules.ModuleName, "FrmPhieuXuatKhoVatTu_CS", "msgThemVaoDataTrungGianKhongThanhCong", Commons.Modules.TypeLanguage), "", Windows.Forms.MessageBoxButtons.OK, Windows.Forms.MessageBoxIcon.Warning)
                        Return False
                    End If
                End If
                objTrans.Commit()
                ...
                If (Commons.Modules.sPrivate = "VIFON") Then
                    Try
                        SqlHelper.ExecuteNonQuery(Commons.IConnections.CNStr, "spMailPhieuXuat", objDonHangXuat.MS_DH_XUAT_PT, Commons.Modules.TypeLanguage)
                    Catch ex As Exception
        ...
        Public Function TRFNhapKho(ByVal objTrans As SqlTransaction, ByVal DHX As String, ByVal TrangThai As Integer)
            Try
                ...
                If dttmp.Rows.Count > 0 Then
                    ...
                    If System.Net.Dns.GetHostName.ToUpper().ToString() = "M4SHIL0V3".ToUpper.ToString() Then Return True
                    If System.Net.Dns.GetHostName.ToUpper().ToString() = "DESKTOP-8GL1SRF".ToUpper.ToString() Then Return True
                    XtraMessageBox.Show(dttmp.Rows(0)(3).ToString() + vbCrLf + dttmp.Rows(0)(5).ToString())
                    Return False
                End If
            Catch ex As Exception
        ...
        Public Function UPDATE_DON_HANG_XUAT_CS(ByVal objDonHangXuat As IC_DON_HANG_XUAT_Info, ByVal lstDHXVT As List(Of IC_DON_HANG_XUAT_VAT_TU_Info), ByVal lstCTVTPT As List(Of CHI_TIET_VAT_TU_XUAT_Info)) As Boolean
            Dim sl_ton As Decimal? = 0
            Dim objConnection As New SqlConnection(connectionString)
            objConnection.Open()
            Dim objTrans As SqlTransaction = objConnection.BeginTransaction
            Try
                ...
                SqlHelper.ExecuteNonQuery(objTrans, "H_QL_UPDATE_DON_HANG_XUAT_CS", objDonHangXuat.MS_DH_XUAT_PT,
                    objDonHangXuat.SO_PHIEU_XN, objDonHangXuat.GIO, objDonHangXuat.NGAY, objDonHangXuat.MS_KHO,
                    objDonHangXuat.MS_DANG_XUAT, objDonHangXuat.NGUOI_NHAN, objDonHangXuat.NGAY_CHUNG_TU,
                    objDonHangXuat.SO_CHUNG_TU, objDonHangXuat.MS_PHIEU_BAO_TRI, objDonHangXuat.GHI_CHU,
                    objDonHangXuat.LOCK, objDonHangXuat.LY_DO_XUAT, objDonHangXuat.THU_KHO,
                    IIf(objDonHangXuat.MS_BP_CHIU_PHI <> 0, objDonHangXuat.MS_BP_CHIU_PHI, DBNull.Value),
                    objDonHangXuat.NGUOI_LAP, objDonHangXuat.CAN_CU, objDonHangXuat.THU_KHO_KY, objDonHangXuat.MS_KHO_DEN,
                    objDonHangXuat.MS_MAY, objDonHangXuat.MS_LY_DO_XUAT_KT, objDonHangXuat.MS_KH_NHAN)
                ...
                For i As Integer = 0 To lstDHXVTCTIET.Count - 1
                    ...
                    If tempFlag Then
                        SqlHelper.ExecuteNonQuery(objTrans, "QL_ADD_VI_TRI_KHO_VAT_TU", objVTKHOVATTU.MS_KHO,
                            objVTKHOVATTU.MS_PT, objVTKHOVATTU.MS_VI_TRI, objVTKHOVATTU.MS_DH_NHAP_PT,
                            objVTKHOVATTU.SL_VT, objVTKHOVATTU.ID)
                    Else
                        SqlHelper.ExecuteNonQuery(objTrans, "QL_UPDATE_VI_TRI_KHO_VAT_TU", objVTKHOVATTU.MS_KHO,
                            objVTKHOVATTU.MS_PT, objVTKHOVATTU.MS_VI_TRI, objVTKHOVATTU.MS_DH_NHAP_PT,
                            objVTKHOVATTU.SL_VT, objVTKHOVATTU.ID)
                    End If
                Next
                ...
                For i As Integer = 0 To lstDHXVT.Count - 1
                    ...
                    SqlHelper.ExecuteNonQuery(objTrans, "QL_ADD_DON_HANG_XUAT_VAT_TU", objDHXVT.MS_DH_XUAT_PT,
                        objDHXVT.MS_PT, objDHXVT.SO_LUONG_CTU, objDHXVT.SO_LUONG_THUC_XUAT, objDHXVT.GHI_CHU, objDHXVT.TG_PB, objDHXVT.CHI_PHI)
                    ...
                    If lstCTVTPT.Count > 0 Then
                        ...
                        For j As Integer = 0 To lstDHX_VTCT.Count - 1
                            ...
                            If objDHX_VTCT.SL_VT > 0 Then
                                ...
                                SqlHelper.ExecuteNonQuery(objTrans, "QL_ADD_DON_HANG_XUAT_VAT_TU_CHI_TIET",
                                    objDHXVTCT.MS_DH_XUAT_PT, objDHXVTCT.MS_PT, objDHXVTCT.STT,
                                    objDHXVTCT.MS_DH_NHAP_PT, objDHXVTCT.MS_VI_TRI,
    ...
    Public Class CHI_TIET_VAT_TU_XUAT_Info
        ...
        Public Property SL_VT() As Double
            ...
            End Set
        End Property
        Public Property SL_XUAT() As Double
            Get
                Return _SL_XUAT
            End Get
            Set(ByVal value As Double)
