...
Public Class frmKHTT_ChuyenNamQuaNam
    'iLoaiBC 1 la chuyen, 
    'iLoaiBC 2 la copy
    ...
    Private Sub btnLayDL_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnLayDL.Click
        ...
        Try
            ...
            If iLoai = 1 Then
                commandGetBTDK.CommandText = "SP_GET_DATA_KHBT_NAM_TO_NAM"
            Else
                commandGetBTDK.CommandText = "SP_GET_COPY_KHBT_NAM_TO_NAM"
            End If
    ...
    Private Sub LoadNamData()
        ...
        For i As Integer = 0 To _vtb.Rows.Count - 1
            If _vtb.Rows(i)("Code").ToString = KHNam Then
    ...
    Private Sub frmKHTT_ChuyenThangQuaThang_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        If iLoai = 2 Then Me.Text = Commons.Modules.ObjLanguages.GetLanguage(Commons.Modules.ModuleName, Me.Name, "frmKHTT_CopyNamQuaNam", Commons.Modules.TypeLanguage)
    End Sub
    ...
    Private Sub btnThucHien_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnThucHien.Click
        ...
        Catch ex As Exception
            If iLoai = 1 Then
                MsgBox(Commons.Modules.ObjLanguages.GetLanguage(Commons.Modules.ModuleName, Me.Name, "CHUYEN_SANG_NAM_KO_THANH_CONG", _
                        Commons.Modules.TypeLanguage), MsgBoxStyle.Exclamation)
            Else
                MsgBox(Commons.Modules.ObjLanguages.GetLanguage(Commons.Modules.ModuleName, Me.Name, "msgCopyNamKhongThanhCong", _
    ...
    Private Sub ChuyenDuLieu()
        ...
        Try
            For i As Integer = 0 To dataSelectTMP.ToTable().Rows.Count - 1
                ...
                command.CommandText = "INSERT INTO dbo.KE_HOACH_TONG_THE_CHI_TIET ( MA_HANG_MUC, KH_NAM, ID_TRUOC, KH_TYPE , TU_NGAY , DEN_NGAY , MS_UU_TIEN, CP_VT_NN_NGOAI_DM, CP_VT_NGOAI_DM, CP_THUE_NGOAI) " & _
                "VALUES ('" & dataSelectTMP.ToTable().Rows(i)("HANG_MUC_ID").ToString() & "','" & KHNam & "','" & dataSelectTMP.ToTable().Rows(i)("ID").ToString() & "','1' ,'" & Format(dataSelectTMP.ToTable().Rows(i)("NGAY"), "MM/dd/yyyy") & "','" & Format(dataSelectTMP.ToTable().Rows(i)("NGAY_DK_HT"), "MM/dd/yyyy") & "'" & _
                ...
                commandCV.CommandText = "INSERT INTO dbo.KE_HOACH_TONG_CONG_VIEC_CHI_TIET (MS_MAY, MA_HANG_MUC, MA_CV, MA_BO_PHAN, KH_NAM,ID_TRUOC, KH_TYPE,TU_NGAY , DEN_NGAY , MS_UU_TIEN, TG_KH )" & _
                " SELECT MS_MAY, MA_HANG_MUC, MA_CV, MA_BO_PHAN, '" & KHNam & "' AS KH_NAM, ID AS ID_TRUOC,  '1' AS KH_TYPE ,'" & Format(dataSelectTMP.ToTable().Rows(i)("NGAY"), "MM/dd/yyyy") & "','" & Format(dataSelectTMP.ToTable().Rows(i)("NGAY_DK_HT"), "MM/dd/yyyy") & "','" & dataSelectTMP.ToTable().Rows(i)("MS_UU_TIEN") & "' , TG_KH " & _
                " FROM dbo.KE_HOACH_TONG_CONG_VIEC_CHI_TIET WHERE MA_HANG_MUC = '" & dataSelectTMP.ToTable().Rows(i)("HANG_MUC_ID").ToString() & "' AND KH_TYPE = '1' AND  KH_NAM = '" & cbNam.SelectedValue & "' "
                ...
                commandVTPT.CommandText = "INSERT INTO dbo.KE_HOACH_TONG_CONG_VIEC_PHU_TUNG_CHI_TIET(KH_TYPE, KH_NAM, KH_THANG, KH_TUAN, MS_MAY, HANG_MUC_ID, MS_CV, " & _
                " MS_BO_PHAN, MS_PT, SO_LUONG, GHI_CHU, DON_GIA_KH, TIEN_TE_KH, TY_GIA_KH, TY_GIA_USD_KH, " & _
                " DON_GIA_CUOI, NGAY_LAY_DG_KH )" & _
                " SELECT KH_TYPE,'" & KHNam & "' AS KH_NAM, KH_THANG, KH_TUAN, MS_MAY, HANG_MUC_ID, MS_CV, MS_BO_PHAN, MS_PT, SO_LUONG, GHI_CHU, DON_GIA_KH, TIEN_TE_KH, TY_GIA_KH, TY_GIA_USD_KH, " & _
                " DON_GIA_CUOI, NGAY_LAY_DG_KH " & _
                " FROM dbo.KE_HOACH_TONG_CONG_VIEC_PHU_TUNG_CHI_TIET WHERE HANG_MUC_ID = '" & dataSelectTMP.ToTable().Rows(i)("HANG_MUC_ID").ToString() & "' AND KH_TYPE = '1' AND  KH_NAM = '" & cbNam.SelectedValue & "' "
        ...
        Catch ex As Exception
            sqlTrans.Rollback()
            scon.Close()
            MsgBox(Commons.Modules.ObjLanguages.GetLanguage(Commons.Modules.ModuleName, Me.Name, "CHUYEN_SANG_NAM_KO_THANH_CONG", Commons.Modules.TypeLanguage), MsgBoxStyle.Exclamation)
        End Try
    End Sub
        ...
        Try
            ...
            For i As Integer = 0 To dtChon.ToTable().Rows.Count - 1
                SqlHelper.ExecuteNonQuery(sqlTrans, "spCopyKHTT", dtChon.ToTable().Rows(i)("HANG_MUC_ID").ToString(), cbNam.SelectedValue, _
                    -1, dtChon.ToTable().Rows(i)("MS_MAY").ToString(), Commons.Modules.UserName, 1, KHNam, KHThang)
            Next
            sqlTrans.Commit()
            scon.Close()
            Me.DialogResult = Windows.Forms.DialogResult.OK
