...
namespace VietShape
{
    public partial class ucBaoCaoBaoTriDinhKyTheoNam : DevExpress.XtraEditors.XtraUserControl
    {
        private string currentMonth = DateTime.Now.Month.ToString() + ".1";
        public ucBaoCaoBaoTriDinhKyTheoNam()
        {
            InitializeComponent();
        }
        private void ucBaoCaoBaoTriDinhKyTheoNam_Load(object sender, EventArgs e)
        {
            try
            {
        ...
        private void btnExecute_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                try
                {
                    set = SqlHelper.ExecuteDataset(Commons.IConnections.ConnectionString, "spBaoCaoBaoTriDinhKyTheoNam", datTNam.Text, Commons.Modules.UserName, cboDDiem.EditValue, cboDChuyen.EditValue, -1, cboLMay.EditValue,CboNhomThietBiBTDK.EditValue,cboLoaiCV.EditValue, Commons.Modules.TypeLanguage);
                    dtTmp = set.Tables[0];
                ...
                if (DateTime.Now.Year >= datTNam.DateTime.Year)
                {
                    ...
                    //c2 tao bang tam
                    Commons.Modules.ObjSystems.XoaTable("BTDK" + Commons.Modules.ObjSystems);
                    Commons.Modules.ObjSystems.MCreateTableToDatatable(Commons.IConnections.ConnectionString, "BTDK" + Commons.Modules.UserName, dtTmp, "");

                    Commons.Modules.ObjSystems.XoaTable("BTDKCHON" + Commons.Modules.ObjSystems);
                    Commons.Modules.ObjSystems.MCreateTableToDatatable(Commons.IConnections.ConnectionString, "BTDKCHON" + Commons.Modules.UserName, set.Tables[1], "");
                    //// cập nhật vào tháng
                    try
                    {
                        //for (int i = 1; i < DateTime.Now.Month ; i++)
                        //{
                        //    string sSql = "UPDATE A SET A.["+i+"] = -A.["+i+"] FROM BTDK"+Commons.Modules.UserName+" A INNER JOIN BTDKCHON"+ Commons.Modules.UserName + " B ON B.MS_MAY = A.MS_MAY AND A.MS_LOAI_BT = B.MS_LOAI_BT AND B.CO_BT = 1 AND MONTH(B.NGAY_KE) = "+1+"";
                        //    SqlHelper.ExecuteNonQuery(Commons.IConnections.CNStr,CommandType.Text, sSql);
                        //}
                        //kiểm tra nếu là năm hiện tại thì cập nhật
                        if (DateTime.Now.Year == datTNam.DateTime.Year)
                        {
                            SqlHelper.ExecuteNonQuery(Commons.IConnections.ConnectionString, "spCapNhatBaoCaoBaoTriDinhKy", datTNam.DateTime.Year, "BTDK" + Commons.Modules.UserName, "BTDKCHON" + Commons.Modules.UserName);
                            dtTmp.Clear();
                        }
                        dtTmp.Load(SqlHelper.ExecuteReader(Commons.IConnections.ConnectionString, CommandType.Text, "SELECT * FROM  BTDK" + Commons.Modules.UserName + " ORDER BY STT"));
                    }
                    catch (Exception ex)
                    {
                ...
                //xóa bảng tạm
                Commons.Modules.ObjSystems.XoaTable("BTDK" + Commons.Modules.UserName);
                Commons.Modules.ObjSystems.XoaTable("BTDKCHON" + Commons.Modules.UserName);
                dtTmp.Columns.Remove("MS_LOAI_BT");
                DataTable dt = new DataTable();
                dt = dtTmp.Copy();
                Commons.Modules.ObjSystems.MLoadXtraGrid(grdChung, grvChung, dt, false, true, true, true, false, this.Name);
                grvChung.Columns["MS_MAY"].Caption = Commons.Modules.ObjLanguages.GetLanguage(Commons.Modules.ModuleName, "ucBaoCaoBaoTriDinhKyTheoNam", "MS_MAY", Commons.Modules.TypeLanguage);
                grvChung.Columns["TEN_MAY"].Caption = Commons.Modules.ObjLanguages.GetLanguage(Commons.Modules.ModuleName, "ucBaoCaoBaoTriDinhKyTheoNam", "TEN_MAY", Commons.Modules.TypeLanguage); 
                grvChung.Columns["TEN_LOAI_BT"].Caption = Commons.Modules.ObjLanguages.GetLanguage(Commons.Modules.ModuleName, "ucBaoCaoBaoTriDinhKyTheoNam", "TEN_LOAI_BT", Commons.Modules.TypeLanguage);
                grvChung.Columns["Ten_N_XUONG"].Caption = Commons.Modules.ObjLanguages.GetLanguage(Commons.Modules.ModuleName, "ucBaoCaoBaoTriDinhKyTheoNam", "Ten_N_XUONG", Commons.Modules.TypeLanguage);
                grvChung.Columns["TEN_HE_THONG"].Caption = Commons.Modules.ObjLanguages.GetLanguage(Commons.Modules.ModuleName, "ucBaoCaoBaoTriDinhKyTheoNam", "TEN_HE_THONG", Commons.Modules.TypeLanguage);
                grvChung.Columns["TEN_LOAI_MAY"].Caption = Commons.Modules.ObjLanguages.GetLanguage(Commons.Modules.ModuleName, "ucBaoCaoBaoTriDinhKyTheoNam", "TEN_LOAI_MAY", Commons.Modules.TypeLanguage);
                grvChung.Columns["CHU_KY"].Caption = Commons.Modules.ObjLanguages.GetLanguage(Commons.Modules.ModuleName, "ucBaoCaoBaoTriDinhKyTheoNam", "CHU_KY", Commons.Modules.TypeLanguage);
                grvChung.Columns["NGAY_HC_CUOI"].Caption = Commons.Modules.ObjLanguages.GetLanguage(Commons.Modules.ModuleName, "ucBaoCaoBaoTriDinhKyTheoNam", "NGAY_HC_CUOI", Commons.Modules.TypeLanguage);
        ...
        private void InDuLieu(DataTable dtTmp)
        {
            ...
            try
            {
                ...
                Commons.Modules.MExcel.DinhDang(xlWorkSheet, Commons.Modules.ObjLanguages.GetLanguage(Commons.Modules.ModuleName,
                    this.Name, "bcBaoTriDinhKyTheoNam", Commons.Modules.TypeLanguage) + " " + datTNam.Text
                ...
                Commons.Modules.MExcel.ColumnWidth(xlWorkSheet, 27, "@", true, Dong, 3, Dong, 3);
                #region prb
                prbIN.PerformStep();
                prbIN.Update();
                #endregion
                Commons.Modules.MExcel.ColumnWidth(xlWorkSheet, 15, "@", true, Dong, 4, Dong, 4);
                Commons.Modules.MExcel.ColumnWidth(xlWorkSheet, 27, "@", true, Dong, 5, Dong, 5);
                Commons.Modules.MExcel.ColumnWidth(xlWorkSheet, 27, "@", true, Dong, 6, Dong, 6);
                Commons.Modules.MExcel.ColumnWidth(xlWorkSheet, 20, "@", true, Dong, 7, Dong, 7);
                #region prb
                prbIN.PerformStep();
                prbIN.Update();
                #endregion
                Commons.Modules.MExcel.ColumnWidth(xlWorkSheet, 10, "@", true, Dong, 8, Dong, 8);
                Commons.Modules.MExcel.ColumnWidth(xlWorkSheet, 15, "@", true, Dong, 9, Dong, 9);
