...
{
    public class integrate_Nhap_Kho_to_CMMS_copy
    {
        public integrate_Nhap_Kho_to_CMMS_copy()
        {
        }
        ...
        private void GetDataTransfer()
        {
            try
            {
                ...
                string _sqlExec = "SELECT STT, MS_DH_NHAP_PT,  SO_PHIEU_XN,ISNULL(CONVERT(NVARCHAR(30), GIO, 121), '') AS GIO, ISNULL(ROW_ID,0) AS ROW_ID , ISNULL(CONVERT(NVARCHAR(30), NGAY, 121), '') AS NGAY, " +
                                ...
                                " FROM dbo.TB_NHAP_KHO WHERE ISNULL(DA_CHUYEN,CONVERT(BIT,0)) = CONVERT(BIT, 0)";
        ...
        public string ExecSynchronizeNHAP_KHO_To_DBCMMS()
        {
            try
            {
                ...
                if (Nhap_KhoTransfer.Rows.Count > 0)
                {
                    ...
                    {
                        NHAP_KHO_copy _item = new NHAP_KHO_copy();
        ...
        private bool UpdateToDBCMMS(NHAP_KHO_copy _item)
        {
            try
            {
    ...
    public class NHAP_KHO_copy
    {
        public NHAP_KHO_copy() { }
