...
{
    public class integrate_Xuat_Kho_to_CMMS
    {
        public integrate_Xuat_Kho_to_CMMS()
        {
        }
        ...
        private void GetDataTransfer()
        {
            try
            {
                //integrate_common.cmms_connection = "Server=MASHILOVE;Database=ECOMAINT_INTEGRATIONBHS;User Id=sa;Password=123";
                ...
                " ISNULL(CONVERT(NVARCHAR(30), BAO_HANH_DEN_NGAY, 121), '') AS BAO_HANH_DEN_NGAY, XUAT_XU, " +
                ...
                " FROM dbo.TB_PHIEU_XUAT WHERE ISNULL(DA_CHUYEN,CONVERT(BIT,0)) = CONVERT(BIT, 0) ";
                //AND MS_DH_NHAP_PT IN (SELECT DISTINCT MS_DH_NHAP_PT FROM TB_NHAP_KHO)
        ...
        private bool UpdateToDBCMMS(XUAT_KHO _item)
        {
            try
            {
                //cmms_common.cmms_connection = "Server=MASHILOVE;Database=CMMS_BHSt;User Id=sa;Password=123 ;Connection Timeout=99999";
